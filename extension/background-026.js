...
function connectToAgent() {
  ...
  ws.onmessage = async ({ data }) => {
    if (ws?._flowSocketId !== activeSocketId) return;
    // A page submission can spend several minutes uploading source images and
    // waiting for Flow to expose a real remote job.  Manifest V3 otherwise
    // suspends this service worker after roughly 30 seconds of inactivity,
    // which closes both the content-script message channel and the agent
    // WebSocket.  Periodic extension API activity keeps this one request alive.
...
async function handlePageSubmitVideo(msg) {
  ...
  try {
    ...
    let evidence;
    try {
      evidence = await waitForPageVideoSubmission(tab.id, existingIds, jobPromise, 120000);
    } catch (evidenceError) {
      throw uiError || evidenceError;
    }
...
async function readPageVideoStatus(tabId, mediaId, tileIndex = -1) {
  const injected = await chrome.scripting.executeScript({
    ...
    func: (targetId, requestedIndex) => {
      ...
      return {
        completed: !!tile && (!progress || progressValue >= 100),
        mediaId: targetId,
