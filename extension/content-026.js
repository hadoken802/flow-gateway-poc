...
(function () {
  const inject = () => {
    // Do not mutate Flow's document while its streaming HTML parser is active.
...
chrome.runtime.onMessage.addListener((msg, _, reply) => {
  ...
  if (msg.type === 'SUBMIT_VIDEO_UI') {
    bridgePageEvent('SUBMIT_VIDEO_UI', 'SUBMIT_VIDEO_UI_RESULT', msg.payload, 330000)
      .then(reply)
  ...
  if (msg.type === 'GET_PAGE_VIDEO_STATUS') {
    ...
    const media = [...document.querySelectorAll('flow-video-tile img, flow-video-tile video')].find((item) => (
      item.src.includes(`/image/${mediaId}`) || item.src.includes(`/video/${mediaId}`) || item.src === mediaId
    ));
    const tile = media?.closest('flow-video-tile');
    const progress = tile?.querySelector('.progress-bar');
    const progressValue = progress
      ? Number.parseFloat(progress.style.getPropertyValue('--progress-percent') || '0')
      : 100;
    // Flow keeps the progress element in the completed tile and resets its
    // value to 0%, but hides it with opacity: 0.  Treat only a visible progress
    // bar as active; otherwise completed videos would remain "生成中" forever.
    const progressVisible = !!progress && Number.parseFloat(getComputedStyle(progress).opacity || '1') > 0.01;
    reply({
      completed: !!media && !!tile && (!progressVisible || progressValue >= 100),
      mediaId,
  ...
  if (msg.type === 'GET_PAGE_VIDEO_JOB_IDS') {
    const ids = [...document.querySelectorAll('flow-video-tile img, flow-video-tile video')]
...
async function clickPageVideoDownload(mediaId) {
  ...
  while (Date.now() < deadline && !tile) {
    const media = [...document.querySelectorAll('img, video')]
      .find((item) => item.src.includes(`/image/${mediaId}`) || item.src.includes(`/video/${mediaId}`));
    tile = media?.closest('flow-video-tile') || null;
...
async function clickNewProject() {
  ...
  let target = null;
  while (Date.now() < deadline) {
    target = document.querySelector('button.new-project-button') || [...document.querySelectorAll('button, a, [role="button"]')].find((element) => {
