...
def test_restart_manager_probe_detects_holder_when_log_path_is_directory(tmp_path):
    profile = tmp_path / "profiles" / "TEST-LOCK"
    target_dir = profile / "GPUPersistentCache" / "DawnGraphiteCache" / "abc"
    target = target_dir / "cache.db"
    ready = tmp_path / "holder.ready"
...
"""
    ...
    try:
        ...
        assert result["resource_kind"] == "directory"
        assert result["candidate_count"] >= 1
        assert "abc/cache.db" in result["candidate_relative_paths"]
        holders = result["samples"][0]["holders"]
        holder_pids = {holder["holder_pid"] for holder in holders}
        assert holder_pid in holder_pids
        assert result["summary"]["dawn_lock_probe_holder_classification"] != "no_holder_found"
    finally:
        proc.terminate()
        proc.wait(timeout=10)
    ...
    released = None
    while time.time() < deadline:
        released = probe_dawn_cache_lock(target_dir, profile, samples=(0.0,), sleep=lambda _: None)
        if released["summary"]["dawn_lock_probe_holder_classification"] == "no_holder_found":
            break
        time.sleep(0.1)
    assert released is not None
    assert released["summary"]["dawn_lock_probe_holder_classification"] == "no_holder_found"
