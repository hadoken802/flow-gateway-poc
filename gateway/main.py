...
function zhTaskReason(t){
  const code=String(t.error_code||t.last_error_code||t.last_error_category||'');
  const message=String(t.error_message||t.last_error_message||'');
  const status=String(t.status||'');
  const raw=`${code} ${message} ${status}`.toUpperCase();
  ...
  if(raw.includes('HTTP 502')||code==='502'||raw.includes('GATEWAY_TIMEOUT'))return 'Flow 页面操作超时，或者本地服务暂时没有返回结果，请确认窗口正常后重试。';
  if(status==='failed_before_remote_submit')return '任务在发送到 Flow 之前失败，没有进入远程生成阶段。';
  if(status==='manual_review')return '任务已停止自动处理，需要人工检查后再决定是否重试。';
  return `任务失败，系统暂未提供可识别的中文原因${code?`（错误编号：${code}）`:''}。`;
}
async function showTaskReason(id){const data=await api(`/api/v1/tasks/${id}`),t=data.task||data;alert(zhTaskReason(t))}
